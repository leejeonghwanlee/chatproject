...
public class ChatClient extends Frame implements ActionListener, Runnable{
	...
	public ChatClient() {
		...
		txt_server_ip=new TextField("192.168.0.107", 15);//자신의 ip
		txt_name = new TextField("이정환",15);//접속자 이름
		...
		btn_exit.addActionListener(this);//종료버튼 수신기부착
		btn_send.addActionListener(this);//종료버튼 수신기부착
		txt_input.addActionListener(this);//종료버튼 수신기부착
		...
		Label lblchatTitle = new Label("채팅프로그램 v 1.1",Label.CENTER);
		...
		add(chat, "채팅창");
		cl.show(this, "접속창");
		setBounds(250,250,300,300);//위치크게 동시지정
		setVisible(true);
	}
